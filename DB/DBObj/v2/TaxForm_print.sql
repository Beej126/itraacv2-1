...
/*
 ...
 /_/    \_\____/   \/  \/     /_/    \_\__,_|\__,_|_|  \___|___/___/  |_|\___/ \__, |_|\___|
                                                                               __/ |       
                                                                              |___/        
* TODO: when printing ABW, make sure to implement the *DUTY*STATION* lookup via Package.SponsorGUID as the original address sold to
...
SELECT 
  ...
  @Description = ISNULL(f.[Description], 
    (SELECT TransactionType FROM iTRAAC.dbo.tblTransactionTypes WHERE TransTypeID = @TransactionTypeID)),
  ...
  @SponsorName = p.OriginalSponsorName, --these will only be populated *IFF* the SponsorClient record has had it's name changed after this form was created
...
JOIN iTRAAC.dbo.tblTaxOffices o ON o.TaxOfficeID = p.TaxOfficeID
...
IF (@AuthorizedDependentName IS NULL) SELECT @AuthorizedDependentName = UPPER(LName + ', ' + FName + ' (' + CCode + ')') FROM iTRAAC.dbo.tblClients WHERE RowGUID = @AuthorizedDependentClientGUID
...
ELSE BEGIN
  ...
  ELSE BEGIN
    ...
    UPDATE @t SET DATA = @SponsorName + ' ' + @SponsorCCode WHERE FieldName = 'F12'
    UPDATE @t SET DATA = @AuthorizedDependentName WHERE FieldName = 'F13'
    UPDATE @t SET DATA = @Description WHERE FieldName = 'F14'
    --F15, not used: UPDATE @t SET DATA = @SponsorCCode WHERE FieldName = 'F15' 
